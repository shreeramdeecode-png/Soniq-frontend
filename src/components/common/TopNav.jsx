...
export default function TopNav() {
  ...
  const profile = authProfile || { name: 'User', email: '' };
  ...
  return (
    <nav className="flex items-center justify-between px-8 pt-4.5 relative z-nav font-poppins">
      <div
      ...
      <div className="flex items-center gap-0.5">
        {NAV_ITEMS.map((item) => (
          <button
            ...
            className={cn(
              'py-2 px-4 rounded-pill text-base font-medium cursor-pointer transition-all duration-200 border-none bg-transparent',
              isActive(item.path)
      ...
      <div className="flex items-center gap-3">
        ...
        <div ref={notifRef} className="relative">
          <button
            ...
            className="glass-pill w-8 h-8 rounded-full flex items-center justify-center cursor-pointer relative border-none bg-white/60 hover:bg-white/90"
          >
        ...
        <div ref={userRef} className="relative">
          <button
            ...
            className="glass-pill w-8 h-8 rounded-full flex items-center justify-center cursor-pointer border-none bg-white/60 hover:bg-white/90"
          >
          ...
          {userOpen && (
            <div className="absolute top-full right-0 mt-2 w-[220px] bg-white/95 backdrop-blur-xl rounded-[16px] border border-white/90 shadow-[0_12px_40px_rgba(0,0,0,0.15)] overflow-hidden z-30 animate-[scaleIn_0.15s_ease]">
              <div className="px-4 py-3 border-b border-black/5">
                ...
                <p className="text-2xs text-text-muted">{profile.email}</p>
              </div>
              ...
              <div className="border-t border-black/5 py-1.5">
                <button onClick={() => { logout(); navigate('/login'); setUserOpen(false); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-xs font-medium text-status-danger hover:bg-red-50/50 cursor-pointer transition-colors">
